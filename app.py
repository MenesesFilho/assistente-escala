...
if not OPENAI_API_KEY:
    raise RuntimeError("OPENAI_API_KEY não configurada. Defina no Render > Settings > Environment.")
if not ASSISTANT_ID:
    raise RuntimeError("ASSISTANT_ID não configurado. Defina no Render > Settings > Environment.")
...
def wait_for_run(thread_id: str, run_id: str, timeout_s: int = 1200):
    """Aguarda o Assistente concluir o processamento (timeout maior para Render free)."""
    import time
    start = time.time()
    while True:
        ...
        time.sleep(3.0)  # polling mais espaçado para reduzir carga/memória
...
def gerar():
    try:
        ...
        if not output_file_id:
            steps = client.beta.threads.runs.steps.list(thread_id=thread.id, run_id=run.id, order="desc", limit=10)
        ...
        # --------- 7) Baixar (STREAMING PARA DISCO: menor uso de memória) ---------
        tmpdir = tempfile.gettempdir()
        outfile = os.path.join(tmpdir, "ESCALA_GERADA.xlsx")

        # Usa API streaming do SDK para evitar carregar tudo em memória
        try:
            with client.files.content.with_streaming_response(output_file_id) as resp:
                resp.stream_to_file(outfile)
        except AttributeError:
            # SDK mais antigo: fallback para bytes em memória (menos ideal)
            data = client.files.content(output_file_id).read()
            with open(outfile, "wb") as f:
                f.write(data)

        return send_file(outfile, as_attachment=True, download_name="ESCALA_GERADA.xlsx",
