...
# ============== OpenAI / Assistants config (SAFE) ==============
# Lê valores do ambiente (Render -> Settings -> Environment).
# NÃO exponha sua API key em código público.
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
ASSISTANT_ID   = os.getenv("ASSISTANT_ID")
OPENAI_PROJECT = os.getenv("OPENAI_PROJECT")

# Dicas (NÃO descomente em produção):
# # Seus valores atuais no Render (apenas referência, NÃO COMMITAR):
# # OPENAI_API_KEY: sk-proj-... (fornecida por você)
# # OPENAI_PROJECT: proj_LX6onHxa0Rv4DtCIHYyKXOsg
# # ASSISTANT_ID  : asst_uyCdJQV44NbGqRDFkiDhmmha
...
# OPENAI_PROJECT é recomendado para garantir que a key e o assistente estão no mesmo projeto
...
def gerar():
    try:
        ...
        run = client.beta.threads.runs.create(
            thread_id=thread.id,
            assistant_id=os.getenv("ASSISTANT_ID"),  # usa o ambiente
        )
